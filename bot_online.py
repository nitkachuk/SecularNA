    ...
    while True:
        try:
            ...
            if attempt_count > 1:
                ...
                sent_message = bot.reply_to(message, f'<i>⏳ Секундочку... #{attempt_count} ({err})</i>', parse_mode='HTML')  # ответ 1
                err = ''
            else:
                sent_message = bot.reply_to(message, '<i>⚙️ Секундочку...</i>', parse_mode='HTML')  # ответ 1
