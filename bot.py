...
async def main():
    ...
    role_system = """ По-русски. возьми один случайный жанр поэзии. первой строкой
                      укажи жанр поэзии жирным и оступи вниз. дай 2 четверостишья этого 
                      жанра поэзии и через межстрочные интервалы с разделителями-эмодзи дай 2 
                      предложения про этот жанр поэзии и через межстрочные интервалы с разделителями
                      -эмодзи дай 3 поэта-представителей этого жанра поэзии. на каждого поэта одно
                      маленькое предложение описания поэта. По-русски. """
    title = [ '', '' ]
